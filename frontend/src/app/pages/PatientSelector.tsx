...
const purgeStaleBooking = () => {
  ...
  try {
    ...
    if (
      ...
      booking.status === APP_STATUS.COMPLETED ||
      booking.status === APP_STATUS.CANCELLED ||
      booking.status === APP_STATUS.NO_SHOW
    ) {
...
const resolveActiveAppointment = (data: any): any | null => {
  if (!data) return null;
  if (
    data.hasActiveAppointment === true ||
    data.todayAppointment ||
    (data.activeAppointments && data.activeAppointments.length > 0) ||
    data.nextActiveAppointment
  ) {
    return (
      data.todayAppointment ||
      (data.activeAppointments && data.activeAppointments[0]) ||
      data.nextActiveAppointment ||
      null
    );
  }
...
const fallbackCheckExistingBooking = (selectedPatientId: number | string, navigate: any): boolean => {
  ...
  try {
    ...
    const isActive =
      parsed.status !== APP_STATUS.COMPLETED &&
      parsed.status !== APP_STATUS.CANCELLED &&
      parsed.status !== APP_STATUS.NO_SHOW;
    if (isSelectedPatient && isActive) {
...
// ── Subcomponents to keep PatientSelector CC <= 15 ───────────────────────────
  ...
  <header className="mb-10 animate-in fade-in slide-in-from-top duration-700">
    <div className="flex justify-between items-start mb-6 flex-wrap gap-4">
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-wrap justify-end">
        <button
          onClick={() => navigate("/mediator-info")}
          className="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all group shrink-0"
        >
        ...
        <button
          ...
          className="p-3 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-500 hover:bg-rose-500/20 transition-all"
        >
          <LogOut size={20} />
        </button>
      </div>
    ...
    <div className="mt-2 flex items-center gap-2">
      <div className="h-1 flex-1 rounded-full bg-white/5 overflow-hidden">
        <div
          ...
          style={{ width: `${Math.min(((account?.patients?.length || 0) / 5) * 100, 100)}%` }}
        />
      </div>
      <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">
        {account?.patients?.length || 0}/5 Profiles
      </span>
    </div>
  </header>
);
...
const PatientProfilesList = ({
  ...
  if (!account?.patients || account.patients.length === 0) return null;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500">
          {account.patients.length} Active
        </span>
      </div>
      <div className="space-y-3">
        {account.patients.map((p: Patient) => {
          const isSelected = selectedPatientId.toString() === p.id.toString();
          return (
            <motion.div
              key={p.id}
              whileTap={{ scale: 0.98 }}
            ...
            >
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors shrink-0 ${
                  isSelected ? "bg-emerald-500 text-white" : "bg-white/5 text-slate-400"
                }`}
              >
                <Fingerprint size={24} />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-tighter flex-wrap">
                  ...
                  {p.aadharId && (
                    <>
                      <span className="opacity-30">•</span>
                      <span>Aadhar: ****{p.aadharId?.slice(-4)}</span>
                    </>
                  )}
                  {p.abhaId && (
                    <>
  ...
  >
    <div className="flex justify-between items-center px-6 pt-6 pb-4 border-b border-white/5">
      <div className="flex items-center gap-3">
        ...
        <h3 className="font-black text-emerald-400 uppercase tracking-widest text-sm">
    ...
    <div className="p-6 space-y-4">
      <div className="space-y-1">
        <label htmlFor="patientName" className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">
          Full Name *
        </label>
        <input
          id="patientName"
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-1">
          <label htmlFor="patientAge" className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">
            Age *
          </label>
          <input
            id="patientAge"
        ...
        <div className="space-y-1">
          <label htmlFor="patientGender" className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">
            Sex *
          </label>
          <select
            id="patientGender"
      ...
      <div className="pt-2">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">
            Identity Documents (Fill at least one)
          </span>
        </div>

        <div className="rounded-xl border border-white/5 bg-white/[0.02] overflow-hidden divide-y divide-white/5">
          <div className="px-4 py-3">
            <label htmlFor="patientAadhar" className="text-[9px] font-black uppercase tracking-widest text-emerald-600 block mb-1.5">
              Aadhar Card
            </label>
            <input
              id="patientAadhar"
              ...
              maxLength={12}
              readOnly={!!(editingPatientId && account?.patients.find((p: any) => p.id === editingPatientId)?.aadharId)}
              className={`w-full h-10 px-3 rounded-lg bg-white/5 border border-white/10 focus:border-emerald-500/50 outline-none transition-all placeholder:text-slate-700 text-white text-sm tracking-widest ${
                editingPatientId && account?.patients.find((p: any) => p.id === editingPatientId)?.aadharId
          ...
          <div className="px-4 py-3">
            <label htmlFor="patientAbha" className="text-[9px] font-black uppercase tracking-widest text-sky-600 block mb-1.5">
              ABHA Card (Ayushman Bharat)
            </label>
            <input
              id="patientAbha"
          ...
          <div className="px-4 py-3">
            <label htmlFor="patientUdid" className="text-[9px] font-black uppercase tracking-widest text-amber-600 block mb-1.5">
              UDID Card (Disability / Handicapped)
            </label>
            <input
              id="patientUdid"
...
export default function PatientSelector() {
  ...
  const openEditForm = (p: Patient, e: React.MouseEvent) => {
    ...
    if (
      !aadharData &&
      u?.primaryAadharNumber &&
      (p.name.toLowerCase() === u.fullName?.toLowerCase() || p.name.toLowerCase() === u.username?.toLowerCase())
    ) {
  ...
  const handlePatientContinue = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      const redirected = fallbackCheckExistingBooking(selectedPatientId, navigate);
      if (!redirected) {
  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-lg mx-auto px-6 py-10 pb-40">
                ...
                <button
                  onClick={() => {
                    ...
                    if (u) {
                      ...
                      const isPrimaryAdded = patients.some(
                        (p) => p.name.toLowerCase() === (u.fullName || u.username || "").toLowerCase()
                      );
        ...
        <div
          className={`fixed bottom-0 left-0 right-0 p-6 pb-[calc(1.5rem+env(safe-area-inset-bottom))] bg-gradient-to-t ${
            darkMode ? "from-slate-900 via-slate-900/90" : "from-slate-50 via-slate-50/90"
          } to-transparent pt-20 pointer-events-none z-[100]`}
        >
