...
// ── Subcomponents to keep ManageStaff CC <= 15 ───────────────────────────────
  ...
  <header className="flex flex-row justify-between items-start gap-2">
    <div className="flex items-start gap-3">
      <button
        ...
        className={`p-2 sm:p-3 rounded-2xl border transition-all mt-1 ${
          darkMode ? "bg-white/5 border-white/10 text-slate-400 hover:text-white" : "bg-white border-slate-200 text-slate-500"
        }`}
      >
        <ArrowLeft size={18} />
      </button>
    ...
    <div className="flex items-center gap-2 shrink-0">
      <button
        ...
        className={`p-2 sm:p-3 rounded-2xl border ${darkMode ? "bg-white/5 border-white/10" : "bg-white border-slate-200"}`}
      >
      ...
      <button
        ...
        className="p-2 sm:p-3 rounded-2xl bg-rose-500/10 border border-rose-500/20 text-rose-500"
      >
        <LogOut size={16} />
      </button>
    </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 sm:p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20">
        ...
        <div>
          ...
          <p className={`text-[10px] font-black uppercase tracking-widest ${subText}`}>
            @{hiredMediator.username} · Active Staff
          </p>
        </div>
  ...
  <section className={`p-4 sm:p-8 rounded-[32px] border ${cardBg}`}>
    ...
    <div className="flex flex-row gap-2 mt-2">
      <div className="relative flex-1">
        ...
        <input
          ...
          onChange={(e) => {
            setMediatorUsername(e.target.value);
            setError("");
          }}
    ...
    <div
      className={`mt-4 p-4 rounded-2xl ${
        darkMode ? "bg-amber-500/5 border border-amber-500/10" : "bg-amber-50 border border-amber-200"
      }`}
    >
      <p className={`text-[10px] font-black uppercase tracking-widest ${darkMode ? "text-amber-500/70" : "text-amber-700"}`}>
        ⚠ This is a permanent clinical relationship stored in the database. The assigned mediator will manage your patient
        queue on all future sessions.
      </p>
    </div>
  </section>
);

const HireByTokenSection = ({
  textColor,
  subText,
  cardBg,
  mediatorToken,
  ...
  <section className={`p-4 sm:p-8 rounded-[32px] border ${cardBg}`}>
    <div className="flex items-center gap-3 mb-6">
      ...
      <div>
        <h2 className={`text-sm font-black uppercase tracking-widest ${textColor}`}>
          Find Mediator by Identity Token
        </h2>
        <p className={`text-[10px] uppercase tracking-widest ${subText}`}>
          Paste the mediator's unique identity token
        </p>
      </div>
    </div>
    ...
    <div className="flex flex-col sm:flex-row gap-3">
      <div className="relative flex-1">
        ...
        <input
          ...
          value={mediatorToken}
          onChange={(e) => {
            setMediatorToken(e.target.value);
            setError("");
          }}
...
const ClinicalProtocolNotice = ({ darkMode, subText }: any) => (
  <section
    className={`p-6 rounded-[24px] border ${
      darkMode ? "bg-white/[0.02] border-white/5" : "bg-slate-100 border-slate-200"
    }`}
  >
    <div className="flex items-start gap-3">
      ...
      <div className="space-y-1">
        ...
        <p className={`text-xs ${subText} leading-relaxed`}>
          The assigned mediator will operate the live patient queue on your behalf. They will check patients in, manage
          queue priority, and signal turn notifications. This association persists across all sessions until changed.
        </p>
      </div>
    </div>
  </section>
...
export default function ManageStaff() {
  ...
  const [hiredMediator, setHiredMediator] = useState<{
    assigned: boolean;
    username?: string;
    fullName?: string;
    id?: number;
  ...
  const handleHire = async () => {
    if (!mediatorUsername.trim()) {
      setError("Please enter a mediator username.");
      return;
    }
  ...
  const handleHireByToken = async () => {
    if (!mediatorToken.trim()) {
      setError("Please enter an identity token.");
      return;
    }
    const token = user?.token;
    ...
    >
      ...
      <div className="relative max-w-2xl mx-auto px-6 py-10 space-y-10">
        ...
        <HireByUsernameSection
          ...
          setMediatorUsername={setMediatorUsername}
        ...
        <HireByTokenSection
          ...
          subText={subText}
          cardBg={cardBg}
          mediatorToken={mediatorToken}
          setMediatorToken={setMediatorToken}
          setError={setError}
          handleHireByToken={handleHireByToken}
          tokenLoading={tokenLoading}
          darkMode={darkMode}
        />

        <ClinicalProtocolNotice darkMode={darkMode} subText={subText} />
      </div>
    </div>
  );
}
