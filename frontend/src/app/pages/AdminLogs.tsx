...
import { Activity, CheckCircle, Clock, Globe, Laptop, Server, Smartphone, Copy } from "lucide-react";
...
// ── Pure helpers (extracted to keep LogCard CC ≤ 15) ─────────────────────────

const getSourceStyle = (source: string) => {
    if (source === 'USER_ACTION') return { bg: 'bg-sky-500/20 text-sky-400', border: 'border-sky-500/20 bg-sky-500/5' };
    if (source === 'ACCESS')      return { bg: 'bg-purple-500/20 text-purple-400', border: 'border-purple-500/20 bg-purple-500/5' };
    if (source === 'FRONTEND')    return { bg: 'bg-orange-500/20 text-orange-400', border: 'border-orange-500/20 bg-orange-500/5' };
    return { bg: 'bg-rose-500/20 text-rose-400', border: 'border-rose-500/20 bg-rose-500/5' };
};

const getSourceIcon = (source: string) => {
    if (source === 'USER_ACTION') return <Activity size={24} />;
    if (source === 'ACCESS')      return <Globe size={24} />;
    if (source === 'FRONTEND')    return <Smartphone size={24} />;
...
const copyLogToClipboard = (log: TelemetryLog) => {
    const content = `[${log.source}] ${log.timestamp}\nMessage: ${log.message}\n\nStackTrace:\n${log.stackTrace || 'None'}\n\nURL: ${log.url || 'N/A'}`;
    navigator.clipboard.writeText(content);
};

// ── LogCard component (CC now ≤ 15) ──────────────────────────────────────────
...
const LogCard = ({ log, onMarkResolved }: LogCardProps) => {
    const style = getSourceStyle(log.source);
    const resolvedBorder = log.resolved ? 'border-emerald-500/20 bg-emerald-500/5 opacity-60' : style.border;

    return (
        <div className={`p-6 rounded-2xl border transition-all ${resolvedBorder}`}>
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${style.bg}`}>
                ...
                <div className="flex gap-2">
                    <button onClick={() => copyLogToClipboard(log)} className="px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white flex items-center gap-2 transition-all">
                        <Copy size={14} />
                        <span className="text-xs font-bold uppercase tracking-wider">Copy</span>
                    </button>
                    {!log.resolved && log.source !== 'ACCESS' && log.source !== 'USER_ACTION' && (
                        <button onClick={() => onMarkResolved(log.id)} className="px-4 py-2 rounded-xl bg-white/5 hover:bg-emerald-500/20 hover:text-emerald-400 text-slate-400 text-xs font-bold uppercase tracking-wider transition-all">
                            Mark Resolved
                        </button>
                    )}
                </div>
            ...
            {log.stackTrace && (
                <div className="mt-4 bg-black/60 rounded-xl p-4 overflow-x-auto border border-white/5">
                    <div className="text-[10px] uppercase font-black tracking-widest text-slate-500 mb-2">Stack Trace</div>
                    <pre className="text-[11px] font-mono text-slate-400 leading-relaxed">{log.stackTrace}</pre>
                </div>
            )}
        </div>
    );
...
const filterLog = (log: TelemetryLog, filter: string): boolean => {
    ...
    if (filter === "USER ACTION") return log.source === "USER_ACTION";
    return true;
};
...
export default function AdminLogs() {
    ...
    const API_URL = (import.meta as any).env.VITE_API_URL || "https://online-queue-project.onrender.com";
        ...
        <div className="min-h-screen bg-[#020617] text-white p-8">
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex gap-2">
                        {["ALL", "UNRESOLVED", "FRONTEND", "BACKEND", "ACCESS", "USER ACTION"].map(f => (
                            <button
                                key={f}
                                onClick={() => setFilter(f)}
                ...
                {loading ? (
                    ...
                    <div className="space-y-4">
                        {filteredLogs.map(log => (
                            <LogCard key={log.id} log={log} onMarkResolved={markResolved} />
                        ))}
                    </div>
                )}
            </div>
