...
const resolveEffectiveShiftStartTime = (todayAvailability: any, todayAppointments: any[]): string | null => {
  ...
  if (rawEarliestStr) {
    ...
    if (h * 60 + m < clinicStartMinutes) return rawEarliestStr;
  }

  return todayAvailability?.startTime || null;
};

const buildShiftDateTime = (now: Date, shiftDate: string | null, timeStr: string): Date => {
...
const formatShiftCountdown = (diff: number): string => {
...
const checkShiftOvertime = (now: Date, todayAvailability: any, shiftDate: string | null, patientsCount: number): boolean => {
  if (!todayAvailability?.endTime) return false;
  const endTime = buildShiftDateTime(now, shiftDate, todayAvailability.endTime);
  return now.getTime() > endTime.getTime() && patientsCount > 0;
};
...
): { timeToShift: string; isOvertime: boolean } => {
  const shiftDate: string | null = todayAvailability?.date || null;
  const startTimeStr = resolveEffectiveShiftStartTime(todayAvailability, todayAppointments);

  if (!startTimeStr) {
    return { timeToShift: "Awaiting Schedule", isOvertime: false };
  }

  const shiftTime = buildShiftDateTime(now, shiftDate, startTimeStr);
  let timeToShift: string;
  if (now.getTime() >= shiftTime.getTime()) {
    timeToShift = todayAvailability?.closed ? "Clinic Closed Today" : "READY";
  } else {
    timeToShift = formatShiftCountdown(shiftTime.getTime() - now.getTime());
  }

  const isOvertime = checkShiftOvertime(now, todayAvailability, shiftDate, patientsCount);
...
// ── Subcomponents to reduce DoctorDashboard component Cognitive Complexity ──
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-12 text-center relative w-full h-full">
        ...
        <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="relative z-10 space-y-8">
          ...
          <div>
            <h2 className="text-sm font-black uppercase tracking-[0.4em] text-slate-500 mb-2">
              Shift Readiness Protocol
            </h2>
            <h1 className="text-2xl sm:text-5xl lg:text-8xl font-black tracking-tighter italic text-white leading-none transition-all">
              {timeToShift === "Awaiting Schedule"
                ? "NO SCHEDULE"
                : timeToShift === "READY"
                ? ""
          ...
          <p className="text-xs font-bold text-slate-600 max-w-md mx-auto uppercase tracking-widest pt-12">
            Dashboard will automatically activate at shift launch.
            <br />
    ...
    >
      <div className="w-[95%] sm:max-w-lg p-6 sm:p-12 rounded-[64px] bg-rose-600 text-white shadow-[0_20px_100px_rgba(225,29,72,0.6)] relative max-h-[85vh] overflow-y-auto border-4 border-white/20 text-center mx-auto">
        ...
        <p className="text-xl font-bold text-white/90 mb-10 leading-relaxed uppercase tracking-widest">
          ...
          <span className="text-3xl text-white underline decoration-white/40">
            {emergencyData?.patientName || "Priority Patient"}
          </span>
        </p>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
    ...
    >
      ...
      <div>
        ...
        <h3 className={`text-lg font-black tracking-tight ${textColor} leading-tight mt-1`}>
    ...
    >
      ...
      <div>
        ...
        <h3 className="text-3xl font-black tracking-tighter text-indigo-500 mt-1">
...
const DoctorActivePatientCard = ({
  ...
  <div
    className={`${glassBg} p-5 sm:p-10 rounded-[48px] border ${borderCol} min-h-[500px] max-w-4xl mx-auto flex flex-col justify-center relative overflow-hidden shadow-2xl shadow-blue-500/5`}
  >
    <div
      className={`absolute top-0 right-0 p-4 sm:p-12 opacity-[0.03] ${
    ...
    {activePatient ? (
      <div className="relative z-10 flex flex-col items-center text-center space-y-10">
        <div className="space-y-4">
          ...
          <h2
            className={`text-3xl sm:text-4xl md:text-6xl lg:text-8xl font-black tracking-tighter ${textColor} drop-shadow-2xl`}
          >
          ...
          <p className={`font-bold uppercase tracking-[0.4em] text-[10px] ${subTextColor} opacity-60`}>
            Target Diagnostic:{" "}
            <span className="text-blue-500 font-black italic">{activePatient.diagnosis || "Pending Evaluation"}</span>
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-6 w-full max-w-5xl">
          {[
            {
              label: "Patient Age",
              val: activePatient.patientAge || activePatient.age || "N/A",
              icon: <Users size={24} className="text-emerald-500" />,
            },
            {
              label: "Service Tier",
              val: String(activePatient.tier).toUpperCase() === "PREMIUM" ? "Elite" : "Standard",
        ...
        <div className="grid grid-cols-1 w-full max-w-2xl pt-8 border-t border-slate-500/5">
          ...
          <button
            onClick={() =>
              sendMessage(JSON.stringify({ type: "RESTORE_TO_QUEUE", patientId: activePatient.id.toString() }))
            }
      ...
      <div className="text-center py-20 opacity-30 flex flex-col items-center gap-6">
        ...
        <div className="space-y-2">
          ...
          <p className={`text-[10px] font-bold uppercase tracking-widest ${subTextColor} opacity-60`}>
            Signal readiness to the mediator to begin next session
          </p>
        </div>
      </div>
    )}
...
const DoctorPatientQueueSection = ({
  sortedPatients,
  activePatient,
  safeId,
  setSelectedPatient,
  ...
  <div className="space-y-12">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {sortedPatients
        .filter((p: any) => !activePatient || safeId(p, -1) !== safeId(activePatient, -1))
        .filter((p: any) => p.status !== APP_STATUS.IN_SESSION)
        .map((p: any, i: number) => (
          <motion.div
            key={safeId(p, i)}
            layout
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span
                    className={`text-[9px] font-black uppercase tracking-widest ${
                      p.status === APP_STATUS.ARRIVED ? "text-emerald-500" : subTextColor
                    }`}
                  >
                    {p.status}
                  </span>
...
const DoctorPatientDetailModal = ({
  selectedPatient,
  setSelectedPatient,
  glassBg,
  borderCol,
        ...
        <div className="relative z-10 space-y-8">
          <div className="flex justify-between items-start">
            <div className="space-y-2">
              ...
              <h2 className={`text-3xl sm:text-5xl font-black tracking-tighter ${textColor}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="p-6 rounded-3xl bg-white/5 border border-white/5">
              ...
              <p className="text-2xl font-black text-white italic">
                {selectedPatient.patientAge || selectedPatient.age || "N/A"} Years
              </p>
            </div>
            <div className="p-6 rounded-3xl bg-white/5 border border-white/5">
              ...
              <p
                className={`text-2xl font-black italic ${
                  String(selectedPatient.tier).toUpperCase() === "PREMIUM" ? "text-amber-500" : "text-blue-400"
                }`}
              >
...
export default function DoctorDashboard(): React.JSX.Element {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          const todayAppts = data.filter(
            (a: any) =>
              a.date === targetDate &&
              ![APP_STATUS.COMPLETED, APP_STATUS.CANCELLED, APP_STATUS.NO_SHOW].includes(a.status)
          );
          setTodayAppointments(todayAppts);

          const premium = todayAppts.filter(
            (a: any) => String(a.tier).toUpperCase() === "PREMIUM" || a.isPremium
          ).length;
          setRevenueStats({
            premium,
  ...
  }, [navigate, todayAvailability, todayAppointments, isOvertime, hasPlayedOvertimeChime, patients.length, playOvertimeChime]);

  const { lastJsonMessage: queueMessage, sendMessage } = useWebSocket(wsUrl, {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (Array.isArray(data)) {
          setPatients(data.filter((p: any) => String(p.doctorId) === String(user?.doctorId || user?.id)));
        }
      }
    } catch (e) {
  ...
  const handleDischarge = useCallback(
    (id: number | string) => {
      ...
      setTodayAppointments((prev) =>
        prev.filter((p: any) => String(p.patientId) !== String(id) && String(p.id) !== String(id))
      );

      trackUserAction("CONSULTATION_ENDED", `Patient ID: ${id} discharged by Doctor`);
      sendMessage(JSON.stringify({ type: "DISCHARGE", patientId: id.toString() }));
    },
    [sendMessage]
  );
  ...
  const activePatient = useMemo(
    () => safePatients.find((p) => p.status === APP_STATUS.IN_SESSION),
    [safePatients]
  );

  const sortedPatients = useMemo(() => {
    return [...safePatients].sort((a, b) => (a.tokenNumber || 0) - (b.tokenNumber || 0));
  }, [safePatients]);
    ...
    >
      ...
      <AnimatePresence>
        <DoctorShiftOverlay
          ...
          handleDoctorReady={handleDoctorReady}
          navigate={navigate}
        />
      </AnimatePresence>
      ...
      <main className="relative max-w-6xl mx-auto px-6">
        <AnimatePresence>
          <DoctorEmergencyModal
            ...
            emergencyAudio={emergencyAudio}
            setEmergencyAudio={setEmergencyAudio}
          />
        </AnimatePresence>
        ...
        <div className="max-w-5xl mx-auto space-y-12">
          ...
          {todayAppointments.length > 0 && !shiftStarted && (
            <div
              className={`${glassBg} p-5 sm:p-10 rounded-[48px] border ${borderCol} max-w-4xl mx-auto flex flex-col items-center justify-center relative overflow-hidden shadow-2xl text-center mb-12`}
            >
